...
// Background and killed app notifications are handled automatically by the system
...
export async function scheduleSessionReminder(
  ...
  try {
    ...
    const existingNotificationId = await getNotificationIdForSession(sessionId);
    if (existingNotificationId) {
      // Check if this notification is still scheduled
      const scheduledNotifications =
        await Notifications.getAllScheduledNotificationsAsync();
      const isStillScheduled = scheduledNotifications.some(
        (n) => n.identifier === existingNotificationId
      );

      if (isStillScheduled) {
        console.log(
          `Notification already scheduled for "${sessionTitle}". Skipping duplicate.`
        );
    ...
    if (secondsFromNow < 60) {
      console.log(
        `Reminder time for "${sessionTitle}" is too close (${secondsFromNow}s) or in past. Skipping notification.`
      );
      return null;
    }

    console.log(
      `Scheduling notification for "${sessionTitle}" in ${secondsFromNow} seconds (at ${reminderTime.toLocaleString()})`
    );
...
async function saveNotificationMapping(
  sessionId: string,
  notificationId: string
): Promise<void> {
  try {
    const existing = await AsyncStorage.getItem(NOTIFICATION_STORAGE_KEY);
    const mappings: SessionNotification[] = existing
      ? JSON.parse(existing)
      : [];
...
async function getNotificationIdForSession(
  sessionId: string
): Promise<string | null> {
  try {
    const existing = await AsyncStorage.getItem(NOTIFICATION_STORAGE_KEY);
